...
t.test('log replication - the leader can append entries', function (t) {
  ...
  createClusterWithLeader({clusterSize: 5}, function (err, cluster, leader, cleanup) {
    ...
    t.ok(leader, 'a leader was elected, and all nodes are in consensus ' + leader.id )
    ...
    for (var i=0; i<5; ++i) {
    leader.append('foobar' +i.toString())
    }
    leader.append('foobar' +i.toString())
    .then(function () {
      t.pass('should append the message')

      t.equals(leader.getLog().length, 6, 'there should be a log of length 6')
      console.log(leader.getLog())
      t.equals(leader.getCommitIndex(), 5, 'the commit index should be 5')
