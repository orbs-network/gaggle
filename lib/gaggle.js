...
Gaggle.prototype.getBlocks = async function (start, end) {
...
Gaggle.prototype._onMessageRecieved = function _onMessageRecieved (originNodeId, data) {
  ...
  // Important: Optimized to interact with ORBS node logic -> triggers empty commit on commitIndex
  if (self._lastApplied < self._commitIndex) {
    var indices = Array(self._commitIndex - self._lastApplied).fill().map((_, idx) => self._lastApplied + 1 + idx)
    self._lastApplied = self._commitIndex;
    // self._emitter.emit('committed', JSON.parse(JSON.stringify(self._log.slice(self._lastApplied + 1, self._commitIndex + 1))), indices);
    self._emitter.emit('committed');
  }
