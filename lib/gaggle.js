...
Gaggle.prototype._onMessageRecieved = function _onMessageRecieved (originNodeId, data) {
  ...
  for (i=self._lastApplied + 1, ii=self._commitIndex; i<=ii; ++i) {
    console.log('Gaggle:', self.id, 'term', self._currentTerm, 'emitting committed');
...
Gaggle.prototype._handleMessage = function _handleMessage (originNodeId, data) {
  ...
  switch (data.type) {
    ...
    if (data.term === self._currentTerm &&
      ...
      (_.values(self._votes).length >= Math.floor(this._clusterSize/2) + 1) // Wait for a majority
      ) {
    ...
    console.log('Gaggle:', self.id, 'term', self._currentTerm, 'received new data entries', _.size(data.entries), 'from', data.leaderId, 'term', data.term);
    console.log('Gaggle:', self.id, 'term', self._currentTerm, 'log size is', _.size(self._log));
    ...
    if (data.term >= self._currentTerm && self._state !== STATES.LEADER) {
      ...
      console.log('Gaggle:', self.id, 'term', self._currentTerm, 'emitted leaderElected');
    }
    ...
    if (data.term < self._currentTerm) {                        // 1. reply false if term < currentTerm (section 3.3)
      console.log('Gaggle:', self.id, 'term', self._currentTerm, 'could not append entries:', data.term, '<', self._currentTerm);
    ...
    if (data.prevLogIndex > -1 &&                                  // Don't do this if log is empty; it'll fail for the wrong reasons
        ...
        console.log('Gaggle:', self.id, 'term', self._currentTerm, 'could not append entries:', 
          data.prevLogIndex > -1, 
    ...
    console.log('Gaggle:', self.id, 'term', self._currentTerm, 'log after merging new entries', _.size(self._log));
    ...
    console.log('Gaggle:', self.id, 'term', self._currentTerm, 'new commit index is', _.size(self._commit));
