...
Gaggle.prototype._handleMessage = function _handleMessage (originNodeId, data) {
  ...
  switch (data.type) {
    ...
    if (data.term === self._currentTerm &&
      ...
      (_.values(self._votes).length >= Math.floor(this._clusterSize/2)) // Wait for a majority - including itself -> decrease one
      ) {
    ...
    console.log('Gaggle:', self.id, 'term', self._currentTerm, '(current leader:', self._leader, ')', 'received new data entries', _.size(data.entries), 'from', data.leaderId, 'term', data.term);
    console.log('Gaggle:', self.id, 'term', self._currentTerm, '(current leader:', self._leader, ')', 'log size is', _.size(self._log));


    // This is how you lose an election
    if (data.term >= self._currentTerm && self._state !== STATES.LEADER) {
      console.log('Gaggle:', self.id, 'term', self._currentTerm, '(This is how you lose an election:', originNodeId,  data.term, ')');
      self._state = STATES.FOLLOWER
      self._leader = originNodeId
      self._emitter.emit('leaderElected')

      console.log('Gaggle:', self.id, 'term', self._currentTerm, '(current leader:', self._leader, ')', 'emitted leaderElected');
    }
    ...
    if (data.term < self._currentTerm) {                        // 1. reply false if term < currentTerm (section 3.3)
      console.log('Gaggle:', self.id, 'term', self._currentTerm, '(current leader:', self._leader, ')', 'could not append entries:', data.term, '<', self._currentTerm);
    ...
    if (data.prevLogIndex > -1 &&                                  // Don't do this if log is empty; it'll fail for the wrong reasons
        ...
        console.log('Gaggle:', self.id, 'term', self._currentTerm, '(current leader:', self._leader, ')', 'could not append entries:', 
          data.prevLogIndex > -1, 
          '&& (', self._log[data.prevLogIndex], '==', null, '||', self._log[data.prevLogIndex].term, '!==', data.prevLogTerm, ')' );
        self._channel.send(originNodeId, {
          type: RPC_TYPE.APPEND_ENTRIES_REPLY
        , term: self._currentTerm
        , success: false
    ...
    if (conflictedAt > -1) {
      self._log = self._log.slice(0, conflictedAt)  // TODO:seemed wrong
    }
...
Gaggle.prototype._beginElection = function _beginElection () {
  // TODO: fire only once on timeout
  // condition on "follower\candidate"? 
  ...
  , lastLogTerm: (lastLogIndex > -1 &&  lastLogIndex < this._log.length) ? this._log[lastLogIndex].term : -1
